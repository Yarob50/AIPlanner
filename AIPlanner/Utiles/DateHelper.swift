...
public extension Date {
    ...
    func toStringWithRelativeTime(strings:[RelativeTimeStringType:String]? = nil) -> String {
        ...
        if d < 28 {
            if isPast { // Changed compare to compare2 because it's make conflict with swiftDate labiry 
                if compare2(.isLastWeek) {
        ...
        if compare2(.isThisYear) {
            if isPast {
                if compare2(.isLastMonth) {
    ...
    /// compare2s dates to see if they are equal while ignoring time.
    func compare2(_ comparison:DateComparisonType) -> Bool {
        switch comparison {
        case .isToday:
            return compare2(.isSameDay(as: Date()))
        case .isTomorrow:
            let comparison = Date().adjust(.day, offset:1)
            return compare2(.isSameDay(as: comparison))
        case .isYesterday:
            let comparison = Date().adjust(.day, offset: -1)
            return compare2(.isSameDay(as: comparison))
        ...
        case .isThisWeek:
            return self.compare2(.isSameWeek(as: Date()))
        case .isNextWeek:
            let comparison = Date().adjust(.week, offset:1)
            return compare2(.isSameWeek(as: comparison))
        case .isLastWeek:
            let comparison = Date().adjust(.week, offset:-1)
            return compare2(.isSameWeek(as: comparison))
        ...
        case .isThisMonth:
            return self.compare2(.isSameMonth(as: Date()))
        case .isNextMonth:
            let comparison = Date().adjust(.month, offset:1)
            return compare2(.isSameMonth(as: comparison))
        case .isLastMonth:
            let comparison = Date().adjust(.month, offset:-1)
            return compare2(.isSameMonth(as: comparison))
        ...
        case .isThisYear:
            return self.compare2(.isSameYear(as: Date()))
        case .isNextYear:
            let comparison = Date().adjust(.year, offset:1)
            return compare2(.isSameYear(as: comparison))
        case .isLastYear:
            let comparison = Date().adjust(.year, offset:-1)
            return compare2(.isSameYear(as: comparison))
        case .isSameYear(let date):
            return component(.year) == date.component(.year)
        case .isInTheFuture:
            return self.compare2(.isLater(than: Date()))
        case .isInThePast:
            return self.compare2(.isEarlier(than: Date()))
